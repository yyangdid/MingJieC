...
#+end_src
*** 单目运算符
+ *list 2-4:* 对读取的整数值进行符号取反操作，并输出结果
  #+attr_html: :width 238px
...
+ *list 13-1:* 打开与关闭文件
  #+attr_html: :width 238px
  [[file:images/11301.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/list1301.c
// Description : 打开与关闭文件
...
#+end_src
+ *ex 13-1:* list 13-1(chap13/list1301.c) 中的程序只能打开名为 “abc” 的文件。
  请将程序改为从键盘读入文件名，如果存在该名称的文件，就显示 “该文件存在。”，
  否则就显示 “该文件不存在”。
  #+attr_html: :width 238px
  [[file:images/11302.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/ex1301.c
// Description : list 13-1(chap13/list1301.c) 中的程序只能打开名为 “abc” 的文件。
//               请将程序改为从键盘读入文件名，如果存在该名称的文件，就显示 “该文件存在。”，
//               否则就显示 “该文件不存在”。
...
#+end_src
+ *ex 13-2:* 编写程序，从键盘读入文件名，消去该名称的文件的内容。
  ·以只写模式打开文件即可（用只写模式 "w" 打开文件后，文件的内容就被消除了）。·
  #+attr_html: :width 330px
  [[file:images/11303.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/ex1302.c
// Description : 编写程序，从键盘读入文件名，消去该名称的文件的内容。
//               ·以只写模式打开文件即可（用只写模式 "w" 打开文件后，文件的内容就被消除了）。·

// "w" :如果文件不存在则创建新的空白文件，如果存在则清空文件内容。
...
#+end_src
*** 文件数据汇总
+ *list 13-2:* 读入身高和体重，计算并显示他们的平均值
  #+attr_html: :width 238px
  [[file:images/11304.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/list1302.c
// Description : 读入身高和体重，计算并显示他们的平均值
...
#+end_src
+ *ex 13-3:* 改写 list 13-2(chap13/list1302.c) 中的程序，将文件读入的个人信息按身高排序后显示。
  #+attr_html: :width 238px
  [[file:images/11305.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/ex1303.c
// Description : 改写 list 13-2(chap13/list1302.c) 中的程序，将文件读入的个人信息按身高排序后显示。
...
int main (void)
{
  ...
  int i;
  int renshu = 0;               // 人数
  double hsum = 0.0;            // 身高总和
  double wsum = 0.0;            // 体重总和

  fp = fopen("hw.dat", "r");
  if (fp == NULL)
    printf("文件打开失败！\n");
  else {
    while (fscanf(fp, "%s%lf%lf", hm[renshu].name, &hm[renshu].height, &hm[renshu].weight) == 3) {
      hsum += hm[renshu].height;
      wsum += hm[renshu].weight;
      renshu++;
    }
    sort_by_height(hm, renshu);
    for (i = 0; i < renshu; i++) {
      printf("%-10s %5.1lf %5.1lf\n", hm[i].name, hm[i].height, hm[i].weight);
    }
    printf("----------------------\n");
...
#+end_src
*** 写入日期和时间
+ *list 13-3:* 向文件写入程序运行时的日期和时间
  #+attr_html: :width 238px
  [[file:images/11306.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/list1303.c
// Description : 向文件写入程序运行时的日期和时间

// 个人感觉这个例子有些问题，例子使用了只写模式 "w"，用只写模式打开文件时，如果文件存在则先清空文件内容，
// 如果文件不存在则会创建文件，以目前掌握的知识来看应该不会出现文件打开失败的情况
...
#+end_src
+ *ex 13-4:* 请采用 list 13-2(chap13/list1302.c) 的文件写入形式，
  编写一个从键盘读取姓名、身高和体重，并将这些数据写入文件的程序。
  #+attr_html: :width 460px
  [[file:images/11307.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/ex1304.c
// Description : 请采用 list 13-2(chap13/list1302.c) 的文件写入形式，
//               编写一个从键盘读取姓名、身高和体重，并将这些数据写入文件的程序。
...
int main (void)
{
  ...
  fp = fopen("ex1304.dat", "w");

  do {
    printf("请输入要添加的人员信息：\n");
    printf("姓名：");  scanf("%s", name);
    printf("身高：");  scanf("%lf", &height);
    printf("体重：");  scanf("%lf", &weight);

    fprintf(fp, "%s %.0lf %.1lf\n", name, height, weight);
    printf("%s 的信息添加完成，是否继续添加？【Yes···1 / No···0】", name);
    scanf("%d", &i);
  } while (i == 1);

  puts("录入的人员信息已写入 ex1304.dat 文件，程序结束。");

  return 0;
}
#+end_src
*** 专题 13-1: 获取当前日期和时间
+ *list 13C-1:* 显示当前日期和时间
  #+attr_html: :width 420px
  [[file:images/11308.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/list13C1.c
...
int main (void)
{
  ...
  printf("当前日期和时间为 %d 年 %d 月 %d 日 (%s) %d 时 %d 分 %d 秒。\n",
         timer->tm_year + 1900,     // 年（加 1900 后求出）
         timer->tm_mon + 1,         // 月（加 1 后求出）
         timer->tm_mday,            // 日
...
#+end_src
*** 获取上一次运行时的信息
+ *list 13-4:* 显示程序上一次运行时的日期和时间
  #+attr_html: :width 205px
  [[file:images/11313.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap/list1304.c
// Description : 显示程序上一次运行时的日期和时间

#include <time.h>
#include <stdio.h>

char data_file[] = "datatime.dat";          // 文件名
...
#+end_src
*** 显示文件内容
+ *list 13-5:* 将标准输入的数据复制到标准输出
  #+attr_html: :width 238px
  [[file:images/11309.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/list1305.c
// Description : 将标准输入的数据复制到标准输出
...
+ *list 13-6:* 显示文件内容
  ...
  [[file:images/11310.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/list1306.c
// Description : 显示文件内容
...
#+end_src
+ *ex 13-6:* 编写程序实现从键盘读入文件名，计算该文件的行数（换行符的个数）并显示在界面上。
  #+attr_html: :width 238px
  [[file:images/11311.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/ex1306.c
// Description : 编写程序实现从键盘读入文件名，计算该文件的行数（换行符的个数）并显示在界面上。
...
#+end_src
+ *ex 13-7:* 编写程序实现从键盘读入文件名，计算该文件的字符数并显示在界面上。
  #+attr_html: :width 238px
  [[file:images/11312.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/ex1307.c
// Description : 编写程序实现从键盘读入文件名，计算该文件的字符数并显示在界面上。
...
int main (void)
{
  ...
  printf("原文件名：");    scanf("%s", sname);
  printf("目标文件名：");  scanf("%s", dname);
...
#+end_src
+ *ex 13-8:* 请参考 list 13-7(chap13/list1307.c) 编写一个程序，
  在界面上显示文件内容的同时执行复制操作（即同时输出到目标文件和界面）。
  #+attr_html: :width 205px
  [[file:images/11313.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/ex1308.c
// Description : 请参考 list 13-7(chap13/list1307.c) 编写一个程序，
//               在界面上显示文件内容的同时执行复制操作（即同时输出到目标文件和界面）。
...
#+end_src
+ *ex 13-9:* 请参考 list 13-7(chap13/list1307.c) 编写一个程序，
  将所有英文小写字母转换为大写字母的同时执行复制操作。
  #+attr_html: :width 205px
  [[file:images/11313.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/ex1309.c
// Description : 请参考 list 13-7(chap13/list1307.c) 编写一个程序，
//               将所有英文小写字母转换为大写字母的同时执行复制操作。
...
int main (void)
{
  ...
  if ((sfp = fopen(sname, "r")) == NULL)
    ...
    if ((dfp = fopen(dname, "w")) == NULL)
      ...
      while ((ch = fgetc(sfp)) != EOF) {
        if (ch >='a' && ch <= 'z') {
          ch = toupper(ch);     // 方法一：使用 ctype.h 头文件中的函数
          // ch += 'A' - 'a';      // 方法二
        }
        fputc(ch, dfp);
      }
...
#+end_src
+ *ex 13-10:* 请参考 list 13-7(chap13/list1307.c) 编写一个程序，将所有大写字母转换为小写字母的同时执行复制操作。
  #+attr_html: :width 205px
  [[file:images/11313.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/ex1310.c
// Description : 请参考 list 13-7(chap13/list1307.c) 编写一个程序，将所有大写字母转换为小写字母的同时执行复制操作。
...
int main (void)
{
  ...
  if ((sfp = fopen(sname, "r")) == NULL)
    ...
    if ((dfp = fopen(dname, "w")) == NULL)
      ...
      while ((ch = fgetc(sfp)) != EOF) {
        if (ch >= 'A' && ch <= 'Z') {
          // ch = tolower(ch);     // 方法一：使用 ctype.h 中的函数
          ch -= 'A' - 'a';      // 方法二
        }
        fputc(ch, dfp);
      }
...
** 文本和二进制
*** 在文本文件中保存实数
+ *list 13-8:* 将圆周率的值写入文本文件，再进行读取
  #+attr_html: :width 350px
  [[file:images/11314.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/list1308.c
// Description : 将圆周率的值写入文本文件，再进行读取

// 本例根据编译器和操作系统的不同，运行结果的精度可能有所不同
...
int main (void)
{
  ...
  printf("从变量 pi 得到的圆周率为 %23.21f\n", pi);

  // 写入操作
  if ((fp = fopen("PI.txt", "w")) == NULL) // 打开文件
    printf("\a文件打开失败！\n");
  else {
    fprintf(fp, "%f", pi);                 // 从 pi 写入
    fclose(fp);                            // 关闭文件
  }
...
#+end_src
*** 在二进制文件中保存实数
+ *list 13-9:* 将圆周率的值写入二进制文件再进行读取
  #+attr_html: :width 350px
  [[file:images/11315.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/list1309.c
// Description : 将圆周率的值写入二进制文件再进行读取
...
#+end_src
+ *ex 13-11:* 编写一个程序，读取含有 10 个 double 型元素的数组的所有元素值。
  #+attr_html: :width 238px
  [[file:images/11316.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/ex1311.c
// Description : 编写一个程序，读取含有 10 个 double 型元素的数组的所有元素值。
...
int main (void)
{
  ...
  for (i = 0; i < 10; i++)      // 为数组赋值
    d[i] = 3.14159265358979323846;

  // 写入二进制文件
  if ((fp = fopen("d.bin", "wb")) == NULL)
    printf("\a文件打开失败！\n");
  else {
...
#+end_src
+ *ex 13-12:* 改写 list 13-4(chap13/list1304.c) 的程序，
  将日期和时间作为 struct tm 型的值直接向二进制文件进行读写操作。
  #+attr_html: :width 205px
  [[file:images/11313.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/ex1312.c
// Description : 改写 list 13-4(chap13/list1304.c) 的程序，
//               将日期和时间作为 struct tm 型的值直接向二进制文件进行读写操作。

#include <time.h>
#include <stdio.h>

char dt_file[] = "dt.bin";      // 文件名
...
#+end_src
*** 显示文件自身
+ *list 13-10:* 用字符和字符编码显示文件内容
  #+attr_html: :width 540px
  [[file:images/11317.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/list1310.c
// Description : 用字符和字符编码显示文件内容
...
#+end_src
+ *ex 13-13:* 改写 list 13-7(chap13/list1307.c) 的程序，
  将文件作为二进制文件进行复制。注意读写时使用 fread 函数和 fwrite 函数。
  #+attr_html: :width 205px
  [[file:images/11313.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/ex1313.c
// Description : 改写 list 13-7(chap13/list1307.c) 的程序，
//               将文件作为二进制文件进行复制。注意读写时使用 fread 函数和 fwrite 函数。
...
+ *总结：* 将标准输入的数据写入文件
  #+attr_html: :width 238px
  [[file:images/11318.png]]
#+begin_src C
//     Charset : UTF-8(LF)
//   File Name : chap13/summary.c
// Description : 将标准输入的数据写入文件
